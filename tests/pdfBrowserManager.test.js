...
describe('PDF browser lifecycle and queue', () => {
  ...
  test('a disconnected shared browser is replaced on the next request', async () => { await manager.getBrowser(); const disconnected=mockBrowser.once.mock.calls.find((call)=>call[0]==='disconnected')[1]; mockBrowser.connected=false; disconnected(); await manager.getBrowser(); expect(mockLaunch).toHaveBeenCalledTimes(2); expect(manager.state().connected).toBe(true); });
  ...
  test('failed work releases its active slot for the next render', async () => { await expect(manager.withRenderSlot(async()=>{throw new Error('render failed');})).rejects.toThrow('render failed'); expect(manager.state().active).toBe(0); await expect(manager.withRenderSlot(async()=>true)).resolves.toBe(true); expect(manager.state().active).toBe(0); });
