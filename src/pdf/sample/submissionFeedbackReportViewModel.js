...
const feedbackText = (value) => typeof value === 'string' ? value.trim() : '';
const id = (value) => String(value?._id || value || '');
const finite = (value) => Number.isFinite(Number(value));
const clamp = (value, min, max) => Math.min(max, Math.max(min, Number(value)));

function evaluationFeedbackItems(evaluation, evaluationCurrent) {
  if (!evaluationCurrent) return [];
  if (evaluation?.scoringAudit?.overallMethod === 'custom_rubric_weighted_total') {
    const criteria = Array.isArray(evaluation?.customRubricScores?.criteria) ? evaluation.customRubricScores.criteria : [];
    const customItems = criteria.filter((criterion) => {
      const selectedLevel = String(criterion?.selectedLevel ?? criterion?.levelTitle ?? '').trim();
      return Boolean(String(criterion?.criterionId || '').trim() && String(criterion?.title || '').trim() && selectedLevel)
        && [criterion?.normalizedWeight ?? criterion?.weight, criterion?.configuredLevelPercentage ?? criterion?.percentage,
          criterion?.weightedPoints].every((value) => Number.isFinite(Number(value)));
    }).map((criterion) => ({
      category: String(criterion.title).trim(),
      feedback: feedbackText(criterion.comment)
    }));
    if (customItems.length) return customItems.filter((item) => item.feedback);
  }
  const rubricScores = evaluation?.rubricScores && typeof evaluation.rubricScores === 'object' ? evaluation.rubricScores : {};
  return AI_FEEDBACK_CATEGORIES.map(([key, category]) => ({ category, feedback: feedbackText(rubricScores?.[key]?.comment) }))
