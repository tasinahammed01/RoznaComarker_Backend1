...
async function getSubmissionWithPermissionsOrThrow({ user, submissionId }) {
  ...
  if (user.role === "student") {
    if (
      String(submission.student?._id || submission.student) !== String(user._id)
    ) {
      throw new ApiError(403, "Forbidden");
    }
  } else if (user.role === "teacher") {
    await uploadService.assertTeacherOwnsClassOrThrow(
      user._id,
      submission.class,
    );
  } else {
  ...
  try {
    ...
    const studentEmail =
      submission.student && typeof submission.student === "object"
        ? String(
            submission.student.email || submission.student.userEmail || "",
          ).trim()
        : "";
    ...
    try {
      const submissionId =
        req.params && req.params.submissionId
          ...
          : "";
      logger.error(
        `[PDF ERROR] PDF generation failed submissionId=${submissionId} message=${err && err.message ? err.message : String(err)}`,
      );
    } catch {
      // ignore
    }
...
async function downloadWorksheetSubmissionPdf(req, res, next) {
  try {
    ...
    } = require("../modules/worksheetPdfGenerator");
    ...
    const submission = await WorksheetSubmission.findById(submissionId)
      ...
      .populate({
        path: "assignmentId",
        populate: { path: "class", select: "_id name teacher" },
      });
    ...
    if (user.role === "student") {
      ...
      const classId =
        submission.assignmentId?.class?._id || submission.assignmentId?.class;
    ...
    const studentEmail =
      student && typeof student === "object"
        ...
        : "";
    const className = submission.assignmentId?.class?.name
      ? String(submission.assignmentId.class.name).trim()
      : "";

    const tmpDir = path.join(os.tmpdir(), "rozna-pdf");
    const outFile = path.join(
      tmpDir,
      `worksheet-submission-${String(submission._id)}-${uuidv4()}.pdf`,
    );
...
async function downloadWorksheetReportPdf(req, res, next) {
  try {
    ...
    const worksheetId =
      req.params && req.params.worksheetId
        ? String(req.params.worksheetId)
        : "";
    ...
    if (String(worksheet.createdBy) !== String(user._id))
      throw new ApiError(403, "Forbidden");
    ...
    // Get teacher info
    const teacher = await User.findById(worksheet.createdBy)
    ...
    const outFile = path.join(
      tmpDir,
      `worksheet-report-${worksheetId}-${uuidv4()}.pdf`,
    );
...
async function downloadFlashcardReportPdf(req, res, next) {
  try {
    ...
    const setId =
      req.params && req.params.setId ? String(req.params.setId) : "";
    ...
    const assignmentId =
      req.query && req.query.assignmentId
        ? String(req.query.assignmentId).trim()
        : "";
    ...
    const cards = (Array.isArray(set.cards) ? set.cards : []).map((card) => {
      ...
      const correctCount = submissions.filter(
        (s) =>
          ...
          s.results.some(
            (r) =>
              r.cardId && String(r.cardId) === cardIdStr && r.status === "know",
          ),
      ).length;
      return {
    ...
    const outFile = path.join(
      tmpDir,
      `flashcard-report-${setId}-${uuidv4()}.pdf`,
    );
