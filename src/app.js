...
// Method 2: PDF/image upload → WorksheetDocument
const { default: worksheetFileRoute } = require("./routes/worksheetFileRoute");
// CRUD for saved WorksheetDocuments (Phase 1-4 format)
const {
  default: worksheetDocumentRoute,
} = require("./routes/worksheetDocumentRoute");
...
app.use(
  ...
  createCorsMiddleware(),
  express.static(path.join(uploadsRoot, "class-banners")),
);
app.use(
  "/uploads/flashcards",
  createCorsMiddleware(),
...
app.use("/api/worksheets/generate/text", worksheetTextRoute);
app.use("/api/worksheets/generate/file", worksheetFileRoute);
...
app.post(
  ...
  (req, res, next) => {
    ...
    req.params.assignmentId =
      req.body && req.body.assignmentId
