...
const { getAIConfig } = require('../src/services/aiGateway.service');

if (process.env.RUN_LIVE_ADAPTIVE_QUALIFICATION !== 'true') {
  process.stdout.write('Adaptive qualification skipped; set RUN_LIVE_ADAPTIVE_QUALIFICATION=true to run.\n');
  process.exit(0);
}
const GLOBAL_CHAIN = getAIConfig().chain;
const MODELS = ['global-chain'];
...
async function run(model, fixture, iteration) {
  ...
  try {
    raw = await aiGeneration.generateChatCompletion(messages, { feature: 'adaptive_qualification', temperature: 0.2, max_tokens: 4000, response_format: { type: 'json_object' },
...
(async () => {
  ...
  const report = JSON.stringify({ generatedAt: new Date().toISOString(), chain: GLOBAL_CHAIN, iterations: ITERATIONS, cases: cases.map(({ name, count }) => ({ name, activities: count })), summaries: MODELS.map((model) => summarize(model, rows.filter((row) => row.model === model))), rows }, null, 2);
